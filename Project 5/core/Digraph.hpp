...
private:
    ...
    std::map<int, DigraphVertex<VertexInfo, EdgeInfo>> graph_info;
    ...
    inline int findMin(const std::vector<int>&, const std::map<int, double>&) const;
    void DFTr(int vertex, std::map<int, bool>& temp) const;
};
...
std::vector<int> Digraph<VertexInfo, EdgeInfo>::vertices() const
{
    ...
    for (it = graph_info.begin(); it != graph_info.end(); ++it)
    {
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>::edges() const
{
    ...
    for (it = graph_info.begin(); it != graph_info.end(); ++it)
    {
        typename std::list<DigraphEdge<EdgeInfo>>::const_iterator iter;
        for (iter = it->second.edges.begin(); iter != it->second.edges.end(); ++iter)
        {
            answer.push_back(std::make_pair(iter->fromVertex, iter->toVertex));
        }
    }
    return answer;
}
...
{
    if (graph_info.count(vertex) == 0)
    {
        throw DigraphException("Invalid vertex");
    }
    std::vector<std::pair<int, int>> answer;
    typename std::list<DigraphEdge<EdgeInfo>>::const_iterator it;
    for (it = graph_info.find(vertex)->second.edges.begin(); it != graph_info.find(vertex)->second.edges.end(); ++it)
    {
        answer.push_back(std::make_pair(vertex, it->toVertex));
    }
    return answer;
}
...
VertexInfo Digraph<VertexInfo, EdgeInfo>::vertexInfo(int vertex) const
{
    ...
    return graph_info.at(vertex).vinfo;
}
...
{
    if (graph_info.count(fromVertex) == 0 || graph_info.count(toVertex) == 0)
    {
        throw DigraphException("Invalid edge");
    }
    std::vector<std::pair<int, int>> temp = edges(fromVertex);
    std::pair<int, int> pair = std::make_pair(fromVertex, toVertex);
    if (std::find(temp.begin(), temp.end(), pair) == temp.end())
    {
        throw DigraphException("Invalid edge");
    }
    typename std::list<DigraphEdge<EdgeInfo>>::const_iterator it;
    for (it = graph_info.at(fromVertex).edges.begin(); it != graph_info.at(fromVertex).edges.end(); ++it)
    {
...
void Digraph<VertexInfo, EdgeInfo>::addVertex(int vertex, const VertexInfo& vinfo)
{
    if (graph_info.count(vertex) == 0)
    {
        DigraphVertex<VertexInfo, EdgeInfo> info = DigraphVertex<VertexInfo, EdgeInfo>{vinfo};
        graph_info.insert(std::pair<int, DigraphVertex<VertexInfo, EdgeInfo>>(vertex, info));
    }
    else
    {
        throw DigraphException("Invalid edge");
...
void Digraph<VertexInfo, EdgeInfo>::addEdge(int fromVertex, int toVertex, const EdgeInfo& einfo)
{
    ...
    graph_info.at(fromVertex).edges.push_back(DigraphEdge<EdgeInfo>{fromVertex, toVertex, einfo});
}
...
{
    if (graph_info.find(vertex) == graph_info.end())
    {
        throw DigraphException("Invalid vertex");
    }
    for (auto it : graph_info)
    {
        for (auto iter = it.second.edges.begin(); iter != it.second.edges.end(); ++iter)
        {
...
void Digraph<VertexInfo, EdgeInfo>::removeEdge(int fromVertex, int toVertex)
{
    ...
    typename std::list<DigraphEdge<EdgeInfo>>::iterator it = graph_info.at(fromVertex).edges.begin();
    while (it != graph_info.at(fromVertex).edges.end())
    {
        if (it->toVertex == toVertex)
        {
            graph_info.at(fromVertex).edges.erase(it);
        }
        it++;
    }
}
...
int Digraph<VertexInfo, EdgeInfo>::edgeCount(int vertex) const
{
    ...
    return graph_info.at(vertex).edges.size();
}
...
bool Digraph<VertexInfo, EdgeInfo>::isStronglyConnected() const
{
    ...
    for (auto i : graph_info)
    {
        ...
        for (auto iter : temp)
        {
            if (temp[iter.first] == false)
            {
                return false;
            }
            else
            {
                temp[iter.first] = false;
            }
    ...
    std::function<double(const EdgeInfo&)> edgeWeightFunc) const
{
    ...
    while (!unvisited.empty())
    {
        ...
        typename std::list<DigraphEdge<EdgeInfo>>::const_iterator iter;
        for (iter = graph_info.at(temp).edges.begin(); iter != graph_info.at(temp).edges.end(); ++iter)
        {
            double distance = shortest[temp] + edgeWeightFunc(iter->einfo);
            if (distance < shortest[iter->toVertex])
            {
                shortest[iter->toVertex] = distance;
                shortestPaths[iter->toVertex] = temp;
            }
        }
    }
...
//Helper Functions
template <typename VertexInfo, typename EdgeInfo>
void Digraph<VertexInfo, EdgeInfo>::DFTr(int vertex, std::map<int, bool>& temp) const
{
    temp[vertex] = true;
    for (auto it : graph_info.at(vertex).edges)
    {
        if (temp[it.toVertex] == false)
        {
            DFTr(it.toVertex, temp);
        }
    }
}
...
template <typename VertexInfo, typename EdgeInfo>
inline int Digraph<VertexInfo, EdgeInfo>::findMin(const std::vector<int>& vertices, const std::map<int, double>& shortest) const
{
    int negInf = INT_MAX;
    double min = negInf;
    int minVertex;
    ...
    {
        if (shortest.at(vertices[i]) < min)
        {
            minVertex = vertices[i];
            min = shortest.at(vertices[i]);
        }
    }
    return minVertex;
}
